...
NEPALI_WEEKDAYS_FULL = ["आईतबार", "सोमबार", "मंगलबार", "बुधबार", "बिहीबार", "शुक्रबार", "शनिबार"]
ENGLISH_WEEKDAYS_FULL = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
NEPALI_WEEKDAYS = ["आईत", "सोम", "मंगल", "बुध", "बिही", "शुक्र", "शनि"]
...
class NepaliCalendarWindow:
    ...
    def on_focus_out(self, event):
        if self._editing_year or CALENDAR_STAY_OPEN:
            return
        self.root.after(80, self._check_and_close)
...
def create_icon():
    bs = get_bs_today()
    size = 48
    bg = (220, 30, 30, 255) if bs["weekday"] in (0, 6) else (22, 30, 160, 255)
    img = Image.new("RGBA", (size, size), bg)
    draw = ImageDraw.Draw(img)

    # Different settings for Nepali vs English
    if USE_NEPALI:
        text = to_nepali_num(bs["day"])
        font_size = 44          # Nepali digits
        y_offset = -11
    else:
        text = str(bs["day"])
        font_size = 36          # English digits (slightly smaller looks better)
        y_offset = -10

    font = None
    for path in [r"C:\Windows\Fonts\Nirmala.ttf", r"C:\Windows\Fonts\mangalb.ttf", r"C:\Windows\Fonts\arial.ttf"]:
        try:
...
def get_tooltip():
    bs = get_bs_today()
    if USE_NEPALI:
        return f"{bs['month_name_np']} {to_nepali_num(bs['day'])}, {NEPALI_WEEKDAYS_FULL[bs['weekday']]}"
    else:
        return f"{bs['month_name_en']} {bs['day']}, {ENGLISH_WEEKDAYS_FULL[bs['weekday']]}"
